 ...
 /**
  ...
  protected function AddAnAddress($kind, $address, $name = '') {
    if (!preg_match('/^(to|cc|bcc|ReplyTo)$/', $kind)) {
      ...
      $this->Lang('Invalid recipient array').': '.$kind;
    ...
    if (!self::ValidateAddress($address)) {
      ...
      if ($this->exceptions) {
       ...
       $this->Lang('invalid_address').': '.$address;
	  // $this->Lang('invalid_address').': '.$address;
      return false;
    }
    if ($kind != 'ReplyTo') {
...
//$toname,$email,$e_subject,$e_content,$style,$email_id
function send_email($title,$content,$sender_add,$receive_add,$id,$send_name){
	global $update_m,$update_n,$update_form,$update_where,$email_style;
    ...
    $mail->from =  "499758326@qq.com";//设置发件人的邮箱地"no-reply-luxbuy@efolix.com";址
    $mail->fromname = $send_name;                       //设置发件人的姓名
    $mail->SMTPAuth = true;                                    //设置SMTP是否需要密码验证，true表示需要
    $mail->Username= "499758326";
    $mail->Password = "xuan550335@";
    $mail->Subject = $title;                                 //设置邮件的标题
    $mail->AltBody = "To view the message, please use an HTML compatible email viewer! - From www.luxbuy.com";                                // optional, comment out and test
	$mail->IsHTML(true);                                    //设置内容是否为html类型
	
	$mail->Body = $sender_add."<br />".$content;                   
    //$mail->WordWrap = 50;                                 //设置每行的字符数
    $mail->AddReplyTo($sender_add,$send_name);     //设置回复的收件人的地址
    $mail->AddAddress("xuan9172@sina.com","jumex");     //设置收件的地址
    if($mail->Send()){
		//发送邮件成功
		echo"dddd";
	   $db = Db::getInstance($_SC2);
	   $sql="update send_email set send_time=".time()." where id={$id}";
